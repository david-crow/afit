...
image_dir <- "/Users/david/Documents/masters/thesis/analysis/r/steering/images/"
...
# identify the optimal time to prediction (tp)s
compute_tp <- function(var, sequence=0:10) {
    opt_e <- compute_e(var)
    ...
    pdf(paste(image_dir, filename, ".pdf", sep=""))
    plot(rho ~ tp, data=simplex_output, type="l", main=colnames(var)[-1], xlab="Time to Prediction (tp)", ylab="Forecast Skill (rho)")
    dev.off()
...
plot_predictions <- function(var, sequence=0:10) {
    opt_e <- compute_e(var)
    opt_tp <- compute_tp(var, sequence)
...
plot_causality <- function(vars=list(steering, left, right), tp=-10:10) {
    ...
    params <- expand.grid(lib_column=var_names, target_column=var_names, tp=tp, stringsAsFactors=FALSE)
    ...
    ggplot(output, aes(tp, rho, colour=direction)) + geom_line() + geom_point() + geom_vline(xintercept=0, linetype="dashed") +
           scale_x_discrete(limits=tp, labels=labels) + labs(x="Time to Prediction (tp)", y="Forecast Skill (rho)")
...
plot_causality_means <- function(var1, var2) {
    ...
    var1_xmap_var2_means <- ccm_means(ccm(df, E=opt_e, num_samples=100, lib_column=var1_label,
                                          target_column=var2_label, lib_sizes=seq(50, 950, by=50), random_libs=TRUE, replace=TRUE))
    var2_xmap_var1_means <- ccm_means(ccm(df, E=opt_e, num_samples=100, lib_column=var2_label,
                                          target_column=var1_label, lib_sizes=seq(50, 950, by=50), random_libs=TRUE, replace=TRUE))
...
run_edm <- function(vars=list(steering, left, right)){
    for (var in vars) {
