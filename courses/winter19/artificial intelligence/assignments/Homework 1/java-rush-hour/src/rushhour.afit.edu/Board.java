...
public class Board
{
  ...
  public int estimate = Integer.MAX_VALUE;
  ...
  public boolean isGoal() {
    // check whether my vehicle is in the exit square
    Piece piece = piece_list[findPiece("X0")];
    return piece.x + 1 == BOARD_EXIT_X && piece.y == BOARD_EXIT_Y;
  }
  ...
  public void estimate() {
    ...
    // count path-cost to this state
    while (temp != null) {
      path_cost++;
      temp = temp.next;
    }

    // we can assume no other east-west vehicle sits between my vehicle and the exit
    // otherwise, there's no possible solution, and this estimate doesn't matter anyway
    // thus, each non-empty square between my vehicle and the exit is necessarily a unique vehicle

    int num_blocking_pieces = 0;
    int num_blocked_blocking_pieces = 0;
    Piece my_piece = piece_list[findPiece("X0")];

    // for each square between my vehicle and the exit square
    for (int i = my_piece.x + 2; i < BOARD_SIZE; i++) {
      int id = theBoard[i][my_piece.y];

      if (id != -1) {
        num_blocking_pieces++;

        Piece blocking_piece = piece_list[id];

        // if a blocking piece can't fit entirely above the exit row and it can't fit entirely below the exit row,
        // then the blocking piece is itself blocked; we know we'll need at least one more move before we can move this one

        boolean blocked_above = false;
        boolean blocked_below = false;

        // if the blocking piece is too big for the space above the exit row, set blocked_above to true
        if (blocking_piece.dy > BOARD_INDEX - BOARD_EXIT_Y) {
          blocked_above = true;
        } else { // otherwise, check whether the blocking piece is blocked above the exit row by another piece
          for (int y = BOARD_EXIT_Y + 1; y <= BOARD_EXIT_Y + blocking_piece.dy; y++) {
            if (theBoard[blocking_piece.x][y] != -1 && theBoard[blocking_piece.x][y] != id) {
              blocked_above = true;
              break;
            }
          }
        ...
        // if the blocking piece is too big for the space below the exit row, set blocked_below to true
        if (blocking_piece.dy > BOARD_EXIT_Y) {
          blocked_below = true;
        } else { // otherwise, check whether the blocking piece is blocked below the exit row by another piece
          for (int y = BOARD_EXIT_Y - 1; y >= BOARD_EXIT_Y - blocking_piece.dy; y--) {
            if (theBoard[blocking_piece.x][y] != -1 && theBoard[blocking_piece.x][y] != id) {
              blocked_below = true;
              break;
        ...
        // a blocking piece is itself blocked if it can't get out of the way of our vehicle
        if (blocked_above && blocked_below) {
          num_blocked_blocking_pieces++;
        }
      }
    }

    // estimate = path_cost; // Breadth-First Search
    // estimate = path_cost + num_blocking_pieces + 1; // A* with Heuristic 1
    estimate = path_cost;// + num_blocking_pieces + num_blocked_blocking_pieces + 1; // A* with Heuristic 2
  }
  ...
  public boolean equals(Object o)
  {
    ...
    // the two boards are the same if the piece locations are the same
    return Arrays.deepEquals(this.theBoard, b.theBoard);
  }
}
