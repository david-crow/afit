...
class MaxCoverageMinRisk:
    ...
    #############################################
    ########## STANDARD SEARCH ELEMENT: HEURISTIC
    #############################################
    ...
    #############################################
    ########## STANDARD SEARCH ELEMENT: SELECTION
    #############################################
    ...
    # remove all but the best k states from the frontier
    @staticmethod
    def local_filter(frontier, explored, k):
        frontier.sort(reverse=True)

        # update the frontier and the explored set
        for i in range(len(frontier) - 1, k - 1, -1):
            explored.append(frontier[i])
            del frontier[i]
    ...
    def a_star(self, w, p):
        ...
        #####################################################
        ########## STANDARD SEARCH ELEMENT: SET OF CANDIDATES
        #####################################################
        ...
        ########################################################
        ########## STANDARD SEARCH ELEMENT: NEXT-STATE GENERATOR
        ########################################################
        ...
        while len(frontier) > 0:
            ...
            #############################################
            ########## STANDARD SEARCH ELEMENT: OBJECTIVE
            #############################################
            ...
            ###############################################
            ########## STANDARD SEARCH ELEMENT: FEASIBILITY
            ###############################################
            ...
            if self.total_covered(world) == self.size * self.size:
                for path in paths:
                    current = path[-1]
                    if current[0] != 0                   \
                        and current[0] != self.size - 1  \
                        and current[1] != 0              \
                        and current[1] != self.size - 1:
            ...
            ############################################
            ########## STANDARD SEARCH ELEMENT: SOLUTION
            ############################################
    ...
    # local search
    def local_beam_search(self, w, p):
        ...
        while len(frontier) > 0:
            ...
            self.local_filter(frontier, explored, 5)
...
def main():
    ...
    for i, problem in enumerate(problems):
        ...
        if method == "deterministic":
            ...
            world, paths = solver.local_beam_search(solver.init_world(), solver.init_paths())
