...
void GLViewDefenseDaemon::onKeyDown( const SDL_KeyboardEvent& key )
{
   ...
   {
      if( this->locationHistory.size() )
      {
         std::cout << "Plotting most recent trajectory...\n";

         if( gp == nullptr )
            gp = new Gnuplot( "../../../shared/gnuplot/bin/gnuplot.exe" );

         //print out vector with x axis inverted
         *gp << "set xrange [] reverse" << std::endl;
         *gp << "plot" << gp->file1d( this->locationHistory ) << "with lines title 'Observed'" << std::endl;
      } else {
         std::cout << "Not enough data to generate trajectory plot...\n";
      }
   }
   else if( key.keysym.sym == SDLK_4 )
   {
      if( this->locationHistory.size() )
      {
         std::cout << "Plotting most recent trajectory with predicted landing...\n";

         if( pf == nullptr )
            pf = new Gnuplot( "../../../shared/gnuplot/bin/gnuplot.exe" );

         // observed points
         std::vector< double > x_pts;
         std::vector< double > y_pts;

         for( auto& l : locationHistory )
         {
            x_pts.push_back( l.first );
            y_pts.push_back( l.second );
         }

         std::vector< double > coefficients = PolyFit::PolyFit( x_pts, y_pts, 2 );
         ...
         std::cout << "Predicted [launch range, impact range] of inbound missile [" << x1 << ", " << x2 << "]\n";

         x_pts.clear();
         y_pts.clear();

         // find new x values - 2.5 seems nice :)
         for( double i = x1; i <= x2; i += 2.5 )
            x_pts.push_back( i );

         y_pts = PolyFit::PolyVal( coefficients, x_pts );
         std::vector< std::pair< double, double >  > predicted;

         for( size_t i = 0; i < x_pts.size(); i++ )
            predicted.push_back( std::make_pair( x_pts[i], y_pts[i] ) );

         // create plot
         *pf << "set xrange [] reverse" << std::endl;
         *pf << "plot" << pf->file1d( this->locationHistory ) << "with lines title 'Observed',"
            << pf->file1d( predicted ) << "with points title 'Predicted'" << std::endl;
      } else {
         std::cout << "Not enough data to generate trajectory prediction plot...\n";
      }
   }
...
void Aftr::GLViewDefenseDaemon::loadMap()
{
...

   {
      ...
      WOTrackingRibbonBase* ribbon = WOTrackingRibbonBase::New( this->interceptor );
      ribbon->setParentWorldObject( this->interceptor );
      ribbon->setPlumblineRatio( UINT_MAX );
      this->interceptor->setTrackingRibbon( ribbon );
      this->interceptor->getChildren().push_back( ribbon );
   }

   {
      ...
      auto onScanDataAvailable = [&](unsigned int id, const std::chrono::system_clock::time_point &t, std::vector<Vector>&& v)
      {
         for (auto& location : v)
         {
            ...
            this->locationHistory.push_back( std::make_pair(
               location.x * cos( location.z * PI / 180.0f ),
               this->dish->getPosition().z - location.x * sin( location.z * PI / 180.0f )
            ) );
         }
      };
      ...
      {
         if ( wo == this->interceptor )
            this->interceptor->currentlyDetectsRadarBeams( true );
      };

      //Add targets, such as the interceptor and perhaps for ease of debugging, the camera, to the sensor's radar scan list
      this->sensor->addPossibleTargetToRadarScanList(this->interceptor, onObjectIlluminated);
...
void GLViewDefenseDaemon::repopulateFromConfFile()
{
   ...
   this->interceptor->setPositionIgnoringAllChildren( initialRedLaunchPos );
