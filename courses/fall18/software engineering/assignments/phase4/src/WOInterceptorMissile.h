...
public:
   ...
   */
   ...
   void setLaunchTime( const std::chrono::system_clock::time_point& time_point );
   void setLaunchPosition( const Vector& position );
   void setDetonationPosition( const Vector& position );
   void setDefaultColor( const Aftr::aftrColor4ub& color );
   ...
   void setTrackingRibbon( WOTrackingRibbonBase* ribbon ) { this->ribbon = ribbon; }
   bool isDetonating() const { return this->state == MISSILE_STATE::DETONATING; }
   void detonate();
...
private:
   ...
   Aftr::aftrColor4ub defaultColor;
};
